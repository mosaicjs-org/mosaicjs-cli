...
export async function installMosaic(options: InstallOptions = { framework: 'react', template: 'basic' }) {
  ...
  if (!await fs.pathExists(packageJsonPath)) {
    console.error(chalk.red('❌ No package.json found. Please run this command in a Node.js project directory.'));
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(chalk.gray('   http://localhost:4000\n'));
    console.log('3. Add new pages:');
    console.log(chalk.gray('   npx @mosaicjs/cli add:page MyPage\n'));
    console.log('4. Add new layouts:');
    console.log(chalk.gray('   npx @mosaicjs/cli add:layout MyLayout\n'));
...
async function updatePackageJson(projectPath: string, options: InstallOptions) {
...
async function createMosaicStructure(projectPath: string, options: InstallOptions) {
  const dirs = [
    'src/client',
...
async function createMosaicNode(projectPath: string, options: InstallOptions) {
...
<body>
  ...
  await fs.writeFile(path.join(projectPath, 'src/nodes/mosaic/MosaicNode.ts'), mosaicNode);
  
  // Update the nodes index file if it exists
  const nodesIndexPath = path.join(projectPath, 'src/Nodes.ts');
  if (await fs.pathExists(nodesIndexPath)) {
    const nodesContent = await fs.readFile(nodesIndexPath, 'utf-8');
    ...
    if (!hasImport) {
      ...
      if (nodesObjectMatch) {
        ...
        await fs.writeFile(nodesIndexPath, updatedContent);
      }
    }
  }
}
...
async function generateVueFiles(projectPath: string, template: string) {
  ...
  console.log(chalk.yellow('Vue support coming soon!'));
}

async function generateBasicPages(projectPath: string) {
...
async function generateDashboardPages(projectPath: string) {
...
export default Dashboard;`;
  ...
  await fs.writeFile(path.join(projectPath, 'src/client/pages/Dashboard.tsx'), dashboardPage);
}

async function generateLayouts(projectPath: string, template: string) {
...
export default DefaultLayout;`;
  ...
  await fs.writeFile(path.join(projectPath, 'src/client/layouts/DefaultLayout.tsx'), defaultLayout);
...
export default DashboardLayout;`;
    ...
    await fs.writeFile(path.join(projectPath, 'src/client/layouts/DashboardLayout.tsx'), dashboardLayout);
  }
}

async function generateComponents(projectPath: string) {
...
export default Link;`;
  ...
  await fs.writeFile(path.join(projectPath, 'src/client/components/Link.tsx'), linkComponent);
}

async function createWorkflows(projectPath: string, options: InstallOptions) {
  // Home page workflow
  const homeWorkflow = {
    name: "home",
    trigger: {
  ...
  await fs.writeJson(path.join(projectPath, 'workflows/json/home.json'), homeWorkflow, { spaces: 2 });
  ...
  await fs.writeJson(path.join(projectPath, 'workflows/json/home-alt.json'), homeAltWorkflow, { spaces: 2 });
  ...
  await fs.writeJson(path.join(projectPath, 'workflows/json/about.json'), aboutWorkflow, { spaces: 2 });
  ...
  if (options.template === 'dashboard') {
    ...
    await fs.writeJson(path.join(projectPath, 'workflows/json/dashboard.json'), dashboardWorkflow, { spaces: 2 });
  }
}

async function createConfigFiles(projectPath: string, options: InstallOptions) {
  // Vite config
...
export default defineConfig({
  ...
  await fs.writeJson(path.join(projectPath, 'tsconfig.client.json'), tsConfig, { spaces: 2 });

  // Update main tsconfig to exclude client files
  const mainTsConfigPath = path.join(projectPath, 'tsconfig.json');
  if (await fs.pathExists(mainTsConfigPath)) {
    const mainTsConfig = await fs.readJson(mainTsConfigPath);
    mainTsConfig.exclude = mainTsConfig.exclude || [];
    if (!mainTsConfig.exclude.includes('src/client/**/*')) {
      mainTsConfig.exclude.push('src/client/**/*');
    }
    await fs.writeJson(mainTsConfigPath, mainTsConfig, { spaces: 2 });
  }
...
@tailwind utilities;`;
  ...
  await fs.ensureDir(path.join(projectPath, 'src/client/styles'));
  await fs.writeFile(path.join(projectPath, 'src/client/styles/globals.css'), cssFile);
} 