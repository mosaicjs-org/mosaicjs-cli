...
export default ${name};`,
        dashboard: (name) => `import React from 'react';
import type { MosaicPageComponent } from '@mosaic/core';
...
async function createProject(projectName, options = { framework: 'react', template: 'basic' }) {
    ...
    try {
        ...
        if (!options.skipInstall) {
            ...
            try {
                ...
                console.log(chalk_1.default.yellow('You can install them manually by running:'));
        ...
        console.log(chalk_1.default.cyan('  https://mosaicjs.dev'));
        console.log('');
    }
    catch (error) {
...
export default defineConfig({
  ...
  ssr: {
    ...
    await fs_extra_1.default.ensureDir(path_1.default.join(projectPath, 'src/nodes'));
    await fs_extra_1.default.ensureDir(path_1.default.join(projectPath, 'src/client/pages'));
    await fs_extra_1.default.ensureDir(path_1.default.join(projectPath, 'src/client/layouts'));
    await fs_extra_1.default.ensureDir(path_1.default.join(projectPath, 'workflows/json'));
...
export default class MosaicNode extends ReactMosaicNode {
  constructor() {
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/nodes/mosaic.ts'), mosaicNode);
...
export async function hasComponent(componentName: string) {
  try {
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/client/entry-server.tsx'), entryServer);
    // Create Home page
...
onMounted(() => {
  ...
  return () => {
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/client/App.vue'), appComponent);
    // Create entry files
...
if (container.innerHTML) {
  ...
  app.mount(container);
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/client/entry-client.ts'), entryClient);
...
export async function hasComponent(componentName: string) {
  try {
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/client/entry-server.ts'), entryServer);
    // Create Home page
...
withDefaults(defineProps<HomeProps>(), {
  message: "Welcome to MosaicJS!"
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/client/pages/Home.vue'), homePage);
    // Create About page
...
import { Link } from '@mosaicjs/vue/client';
    ...
    await fs_extra_1.default.writeFile(path_1.default.join(projectPath, 'src/client/pages/About.vue'), aboutPage);
}
